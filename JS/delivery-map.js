...
async function searchLocation() {
    ...
    // 1. Fetch from Photon API (Async)
    let apiResults = [];
    try {
        // Prioritize current view or shop location
        ...
        const limit = 5;
        const apiUrl = `https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&lat=${biasLat}&lon=${biasLon}&limit=${limit}`;
        ...
        if (data.features) {
            apiResults = data.features.map(f => {
                ...
                const detailParts = [props.street, props.suburb, props.city, props.state, props.postcode, props.country]
                    .filter(p => p && p !== name);
        ...
        console.error("Photon API Search failed:", err);
    }
    
    // Lowercase for local matching
    ...
    // Find matching locations (Local database)
    const localMatches = [];
    ...
    // Combine: Local matches first (high confidence), then API matches
    const allMatches = [...localMatches, ...apiResults];
    
    if (allMatches.length === 0) {
        const fallbackText = queryLower.includes('sector') ? 'Sector 1, Gandhinagar' : 'CG Road, Ahmedabad';
        searchResults.innerHTML = `<div style="padding:10px; text-align:center; color:#999;">No results found. Try: ${fallbackText}</div>`;
        searchResults.style.display = 'block';
        return;
    }
    
    // Display results
    searchResults.innerHTML = allMatches.map((location, index) => `
        <div onclick="selectSearchResult(${location.lat}, ${location.lng})" style="padding:12px; border-bottom:1px solid #ddd; cursor:pointer; background:#f8f9fa; margin-bottom:5px; border-radius:5px;" onmouseover="this.style.background='#e9ecef'" onmouseout="this.style.background='#f8f9fa'">
            <div style="font-weight:600; color:#333; margin-bottom:3px;">📍 ${location.name}</div>
            <div style="font-size:11px; color:#666;">${location.description || 'Click to select'}</div>
        </div>
    `).join('');
