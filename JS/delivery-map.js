...
const DELIVERY_CONFIG = {
    firstKmFree: true,
    chargePer500m: 10, // ₹10 per 500m (you can change this rate)
...
// Reverse geocoding using Photon API (Komoot)
async function reverseGeocodeAddress(lat, lng) {
    try {
        ...
        if (data.features && data.features.length > 0) {
            ...
            // Construct address intelligently
            const parts = [
                props.name,
                ...
                props.postcode,
                props.country
            ].filter(Boolean); // Remote null/undefined/empty strings
...
function updateDeliveryInfo(distance, deliveryCharge) {
    ...
    if (totalWithDeliveryDisplay) {
        totalWithDeliveryDisplay.textContent = '₹' + grandTotal.toFixed(2);
    }
}
...
async function handleLocationSelection(lat, lng) {
    try {
        ...
        if (mapLocationInput) {
             // Show loading state
             mapLocationInput.value = `📍 Fetching address... (${lat.toFixed(4)}, ${lng.toFixed(4)})`;
        }
        ...
        const addressData = await reverseGeocodeAddress(lat, lng);
        
        if (addressData.found) {
            if (mapLocationInput) {
                mapLocationInput.value = `📍 ${addressData.formattedAddress}`;
                mapLocationInput.style.color = '#333';
            }
            
            // Populate Form Fields if available - DISABLED AS PER USER REQUEST
            /*
            if (addressData.details) {
                const props = addressData.details;
                const setVal = (id, val) => {
                ...
                // Smart assignments
                setVal('streetName', (props.street || '') + (props.housenumber ? ' ' + props.housenumber : ''));
                if (!document.getElementById('streetName').value && props.name) {
                     setVal('streetName', props.name);
                }
                
                setVal('city', props.city || props.town || props.village || '');
                setVal('taluka', props.county || props.district || '');
                setVal('district', props.state_district || props.district || '');
                setVal('state', props.state || '');
                setVal('pincode', props.postcode || '');
            }
            */
        } else {
             if (mapLocationInput) {
                mapLocationInput.value = `📍 Location: ${lat.toFixed(4)}, ${lng.toFixed(4)}`;
            }
        }
        
        console.log('Location selected:', { lat, lng, address: addressData.formattedAddress });
...
const WORKING_LOCATIONS = {
        // Shop Location & Variations
        'the nutty choco morsels': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' },
        'nutty choco morsels': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' },
        'nutty choco': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' },
        'thenuttychocomorsels': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' },
        'thenuttychocomosels': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' }, // Common typo
        'nuttychoco': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' },
        'tncm': { lat: 23.1917174, lng: 72.6213802, name: 'The Nutty Choco Morsels (Home Shop)' },
...
// Search for location using Photon API + Local Data
async function searchLocation() {
    ...
    // Show loading state
    searchResults.innerHTML = '<div style="padding:15px; text-align:center; color:#666;"><i class="fa-solid fa-spinner fa-spin"></i> Searching...</div>';
    searchResults.style.display = 'block';
    
    const queryLower = query.toLowerCase();
    
    // 1. Local Search (Fast & Accurate)
    const localMatches = [];
    for (const [key, location] of Object.entries(WORKING_LOCATIONS)) {
    ...
    let apiResults = [];
    try {
        // Bias towards shop location or selected location
        let biasLat = shopLocation[0];
        let biasLon = shopLocation[1];
        if (userSelectedLatLng) {
            biasLat = userSelectedLatLng[0];
            biasLon = userSelectedLatLng[1];
        }
        
        // Use bounding box for Gujarat to improve accuracy
        // Approx Gujarat bbox: 68.1, 20.1, 74.5, 24.7
        const bbox = "68.1,20.1,74.5,24.7";
        
        const limit = 5;
        const apiUrl = `https://photon.komoot.io/api/?q=${encodeURIComponent(query)}&lat=${biasLat}&lon=${biasLon}&limit=${limit}&bbox=${bbox}`;
        
        const response = await fetch(apiUrl);
        const data = await response.json();
        
        if (data.features) {
            apiResults = data.features.map(f => {
                const props = f.properties;
                
                // Strict Filter: Must be in India to be relevant
    ...
    // Filter duplicates based on rounded coordinates
    const uniqueResults = [];
    ...
    const finalResults = uniqueResults.slice(0, 8); // Show max 8 results
    
    if (finalResults.length === 0) {
        searchResults.innerHTML = `<div style="padding:10px; text-align:center; color:#999;">No results found for "${query}"</div>`;
        return;
    }
    
    // Render results
    searchResults.innerHTML = finalResults.map((location) => `
        <div onclick="window.selectSearchResult(${location.lat}, ${location.lng})" style="padding:12px; border-bottom:1px solid #eee; cursor:pointer; display:flex; align-items:center; gap:10px;">
            <div style="background:#f0f0f0; width:30px; height:30px; border-radius:50%; display:flex; align-items:center; justify-content:center; flex-shrink:0;">
                <i class="fa-solid fa-location-dot" style="color:#6b0f1a;"></i>
            </div>
            <div>
                <div style="font-weight:600; color:#333; font-size:0.95rem;">${location.name}</div>
                <div style="font-size:0.8rem; color:#666;">${location.description}</div>
            </div>
        </div>
    `).join('');
}
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                if (deliveryMap) {
                    ...
                    deliveryMarker.bindPopup('📍 Your Current Location').openPopup();
...
function initializeMap() {
    ...
    L.marker(shopLocation, {
        icon: L.divIcon({
            ...
            html: '<div style="background: #6b0f1a; color: white; padding: 8px; border-radius: 50%; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; font-weight: bold; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3);">🏪</div>',
            iconSize: [30, 30],
            iconAnchor: [15, 15]
        })
...
function selectLocationOnMap(lat, lng) {
    ...
    deliveryMarker = L.marker(userSelectedLatLng, {
        icon: L.divIcon({
            ...
            html: '<div style="background: #28a745; color: white; padding: 8px; border-radius: 50%; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; font-weight: bold; border: 2px solid white; box-shadow: 0 2px 6px rgba(0,0,0,0.3);">📍</div>',
            iconSize: [30, 30],
            iconAnchor: [15, 15],
            draggable: true
        })
...
function setupSearchInput() {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('keypress', function(e) {
            if (e.key === 'Enter') {
                e.preventDefault();
                searchLocation(); // Immediate search on Enter
            }
        });
