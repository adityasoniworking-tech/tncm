    ...
    displayItems.forEach(item => {
        ...
        finalHTML += `
            <div class="product-card" style="background:white; padding:15px; border-radius:10px; text-align:center; box-shadow: 0 4px 10px rgba(0,0,0,0.1); width:100%; max-width:300px; justify-self:center; transition: transform 0.3s ease; ${item.inStock === false ? 'opacity: 0.6;' : ''}">
                <img src="${imgUrl}" style="width:100%; height:200px; object-fit:cover; border-radius:8px; margin-bottom:10px;">
                ...
                <div class="product-footer-mobile">
                    ...
                    <div class="action-wrap" style="width:100%">${actionHTML}</div>
                </div>
            </div>`;
    });
...
// --- 8. REVERSE GEOCODING FUNCTION ---
// Convert coordinates to readable address using OpenStreetMap (Free API)
const getAddressFromCoords = async (lat, lng) => {
    try {
        ...
        if (data.features && data.features.length > 0) {
            ...
            return [...new Set(parts)].join(', ') || "Address not found";
        }
...
window.openCart = function() {
    ...
    // Add delivery location section (even if cart is empty for testing)
    const deliveryLocationSection = document.createElement('div');
    deliveryLocationSection.style.cssText = 'margin-bottom: 20px; padding: 15px; background: #f0f8ff; border-radius: 10px; border-left: 4px solid #6b0f1a;';
    deliveryLocationSection.innerHTML = `
        <div style="display: flex; align-items: center; gap: 10px; margin-bottom: 10px;">
            <i class="fa-solid fa-location-dot" style="color: #6b0f1a; font-size: 1.2rem;"></i>
            <strong style="color: #6b0f1a; font-size: 1rem;">Delivery Location</strong>
        </div>
        <div id="deliveryAddress" style="color: #666; font-size: 0.9rem; line-height: 1.4;">
            <i class="fa-solid fa-spinner fa-spin" style="margin-right: 5px;"></i>
            Detecting your delivery address...
        </div>
    `;
    list.appendChild(deliveryLocationSection);
    ...
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                if (addressElement) {
                    addressElement.innerHTML = `
                        <i class="fa-solid fa-map-marker-alt" style="color: #10b981; margin-right: 5px;"></i>
                        ${address}
                    `;
                }
            },
            (error) => {
                ...
                if (addressElement) {
                    addressElement.innerHTML = `
                        <i class="fa-solid fa-exclamation-triangle" style="color: #f59e0b; margin-right: 5px;"></i>
    ...
    if(cartIds.length === 0) {
        // Add separator for empty cart
        const separator = document.createElement('div');
        separator.style.cssText = 'margin: 15px 0; padding-top: 15px; border-top: 1px solid #eee;';
        separator.innerHTML = '<strong style="color: #6b0f1a; font-size: 0.9rem;">Order Items:</strong>';
